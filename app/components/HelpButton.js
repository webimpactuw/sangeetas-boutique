...
export default function HelpButton({ title, ctaLabel, phone }) {
  ...
  return (
    <div className="fixed bottom-5 right-4 md:bottom-8 md:right-8 z-40 flex flex-col items-end gap-3">
      {expanded && (
        <div
          className="relative bg-white text-navy rounded-sm border border-sanji-border pl-4 pr-8 py-3 md:pl-5 md:pr-9 md:py-4 max-w-[240px] shadow-[0_8px_28px_rgba(0,0,0,0.28)]"
          role="dialog"
        ...
        >
          <button
            ...
            className="absolute top-1 right-1.5 w-5 h-5 flex items-center justify-center text-navy/50 hover:text-navy text-base leading-none"
          >
            &times;
          </button>
          ...
          <p className="font-cardo text-xs md:text-sm mt-2 text-navy/80">{ph}</p>
        </div>
      )}
      ...
      <button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-navy text-white flex items-center justify-center ring-4 ring-white shadow-[0_6px_24px_rgba(0,0,0,0.35)] hover:bg-navy/90 transition-colors"
      >
